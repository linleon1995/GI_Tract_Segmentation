...
class SoftIoULoss(nn.Module):
    ...
    def to_one_hot(tensor, n_classes):
        ...
        n, h, w = tensor.size()
        one_hot = torch.zeros(n, n_classes, h, w).to(device).scatter_(1, tensor.view(n, 1, h, w).to(device), 1).to(device)
        return one_hot
...
class _AbstractDiceLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        input = self.normalization(input['out'])
